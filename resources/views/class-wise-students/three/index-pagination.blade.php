...
@if( !$data->isEmpty() )
	...
	@foreach($data as $key => $value)
		...
		<tr id="student_ids{{ $value->id }}">
			...
			<form method="POST" action="{{ route('class-three-single-student-wise-promote-class', $value->id) }}">
				@csrf
			...
			<form method="POST" action="{{ route('class-three-wise-students-demote-status', $value->id) }}">
			@csrf
				<div class="" style="margin-left:10px;">
			...
			@else 
			<form method="POST" action="{{ route('class-three-wise-students-promote-status', $value->id) }}">
			@csrf
				<div class="" style="margin-left:10px;">
			...
			{{-- <form method="POST" action="{{ route('class-three-wise-students.destroy', $value->id) }}">
